# About — architecture, evaluation, and operational detail

This document carries the full technical detail behind the pipeline. The
[README](README.md) is the user-facing overview; start there if you haven't.
...
```
  your handbook PDF  (local only — copyrighted)
        ...
        │                     rewrites + an intent-level reframe (D50; skippable via --no-rewrite;
        │                     degrades to the raw query with no API call when keyless — CI and
        │                     offline runs stay zero-call)
